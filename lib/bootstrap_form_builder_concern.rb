module BootstrapFormBuilderConcern
  ...
  def bs_button(value = nil, options = {}, &)
    ...
    normalized_options.merge!(BootstrapOptions.options_with_button_class(normalized_options))
  ...
  def bs_plain_text_field(method, options = {})
    text_field(
      method,
      BootstrapOptions.options_with_base_class(options, FORM_CONTROL_PLAIN_TEXT_BASE_CLASS).merge(readonly: true)
    )
  end
  ...
  def options_with_form_check_input_class(options)
    BootstrapOptions.options_with_base_class(options, FORM_CHECK_INPUT_BASE_CLASS)
  end

  def options_with_form_control_class(options)
    size = BootstrapOptions.prefixed_option(options, key: :size, prefix: FORM_CONTROL_BASE_CLASS)
    BootstrapOptions.options_with_base_class(options, FORM_CONTROL_BASE_CLASS, size)
  end

  def options_with_form_select_class(options, html_options)
    size = BootstrapOptions.prefixed_option(options, key: :size, prefix: FORM_SELECT_BASE_CLASS)
    BootstrapOptions.options_with_base_class(html_options, FORM_SELECT_BASE_CLASS, size)
  end

  def options_with_form_label_class(options)
    required = REQUIRED_CLASS if options.delete(:required)
    BootstrapOptions.options_with_base_class(options, FORM_LABEL_BASE_CLASS, required)
  end
end
